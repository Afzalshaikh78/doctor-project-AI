...
export async function bookAppointment(formData) {
  ...
  try {
    ...
    try {
      ...
      const appointmentTime = `${format(startTime, "h:mm a")} - ${format(
        endTime,
        "h:mm a"
      )}`;

      // Send confirmation email to patient
      const patientEmailResult = await sendAppointmentConfirmationEmail({
      ...
      const doctorEmailResult = await sendDoctorNotificationEmail({
      ...
      // Don't fail the appointment booking if emails fail
    }
